...
export default function NewPassagePage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(data.error || '지문 등록에 실패했습니다.');
      }
      ...
      setError(err.message || '지문 등록에 실패했습니다.');
    } finally {
      setIsLoading(false);
    }
  };
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6">
      {/* Page Header */}
      <div>
        <h1 className="text-3xl font-bold text-gray-900">지문 등록</h1>
        <p className="text-gray-600 mt-1">새로운 학습 지문을 등록하세요</p>
      </div>
