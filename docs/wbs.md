...
- [ ] 로그인 페이지
  - [ ] 사용자 유형 선택 (교사/학생)
  - [ ] 학생 로그인 폼 (학번 + 아이디 + 비밀번호)
  - [ ] 교사 로그인 폼 (교사 ID + 비밀번호)
  - [ ] 로그인 에러 처리
- [ ] 회원가입 페이지 (관리자가 생성하므로 선택적)
...
**2.6 통계 분석 (4-5일)**
- [ ] 통계 페이지 (`/teacher/statistics`)
  - [ ] 전체 통계 조회 API (`GET /api/statistics`)
  - [ ] 학년별 평균 점수
  - [ ] 반별 평균 점수
  - [ ] 카테고리별 평균 점수
  - [ ] 세부 카테고리별 평균 점수
...
**3.3 학습하기 - 교사 지정 (3-4일)**
- [ ] 지정된 학습 페이지 (`/student/study/assigned`)
  - [ ] 교사가 지정한 지문/문제 목록 조회 API
  - [ ] 완료 기한 표시
  - [ ] 완료 여부 표시
  - [ ] 미완료 항목 우선 표시
- [ ] 지정된 지문 학습 (스스로 선택과 동일한 워크플로우)
- [ ] 완료 상태 업데이트 API

**3.4 문법/개념 풀기 (2-3일)**
- [ ] 문법/개념 선택 페이지 (`/student/study/grammar`)
...
**3.6 오답 노트 (3-4일)**
- [ ] 오답 목록 페이지 (`/student/wrong-answers`)
  - [ ] 오답 목록 조회 API (`GET /api/users/me/wrong-answers`)
  - [ ] 카테고리별 분류
  - [ ] 복습 여부 표시
- [ ] 오답 복습 페이지
  - [ ] 오답 문제만 다시 풀기
...
**3.7 순위 (2-3일)**
- [ ] 순위 페이지 (`/student/ranking`)
  - [ ] 반별 순위 조회 API (`GET /api/ranking/class`)
  - [ ] 학년별 순위 조회 API (`GET /api/ranking/grade`)
  - [ ] 전체 순위 조회 API (`GET /api/ranking/all`)
  - [ ] 상위 5등까지만 표시 (이름 비공개, 점수만)
  - [ ] 본인 순위 별도 표시 (이름 포함)
- [ ] 순위 계산 로직 (평균 점수 기준)
